...
def getTempCredentials(
    ...
    # credentialProvider="http://localhost:8529/_db/_system/_admin/aardvark",
...
def connect_python_arango_local(login: dict):
    url = "http://" + login["hostname"] + ":" + str(login["port"])
    return ArangoClient(hosts=url).db(
        login["dbName"],
        username=login["username"],
        password=login["password"],
